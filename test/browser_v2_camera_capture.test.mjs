...
test("V2 photo picker stays reusable and camera sends captured files through OCR", async () => {
  ...
  try {
    ...
    try {
      ...
      await stripActiveReviewCodeCandidates(cdp);
      ...
      try {
        ...
        await send(reviewsCdp, "Page.addScriptToEvaluateOnNewDocument", {
          source: `
            sessionStorage.setItem("fifa-v2-controller-reload-build-6dcc142be40f", "1");
            window.__reviewErrors = [];
            window.__cloudFetchStarted = false;
            window.__cloudAutosaveStarted = false;
            window.__cloudAppliedCount = 0;
            const nativeFetch = window.fetch.bind(window);
            window.fetch = (input, init) => {
              const url = String(input?.url || input || "");
              if (url.startsWith("https://fifa-collection-sync.enderofworlds007.workers.dev")) {
                window.__cloudFetchStarted = true;
                if (String(init?.method || "GET").toUpperCase() === "POST") {
                  window.__cloudAutosaveStarted = true;
                  return new Promise(() => {});
                }
                return Promise.resolve(new Response(JSON.stringify({ currentRevision: 0, transactions: [] }), {
                  status: 200,
            ...
            window.addEventListener("error", (event) => window.__reviewErrors.push(String(event.error?.message || event.message || "error")));
            window.addEventListener("unhandledrejection", (event) => window.__reviewErrors.push(String(event.reason?.message || event.reason || "rejection")));
            window.addEventListener("panini:cloud-sync-applied", () => { window.__cloudAppliedCount += 1; });
          `,
        });
        ...
        const restoredReview = await evaluate(reviewsCdp, `({
          ...
          candidate: document.querySelector("#photoReviewInspector .photoReviewChoices button")?.textContent,
        ...
        assert.equal(restoredReview.candidate, "TUR5");
        assert.deepEqual(restoredReview.errors, []);

        await waitForExpression(reviewsCdp, `document.querySelector("#restoreCloudIdButton")?.disabled === false`);
        await waitForExpression(reviewsCdp, `window.__cloudFetchStarted === true`);
        await waitForExpression(reviewsCdp, `document.querySelector("#cloudSyncStatus span")?.textContent.includes("Cloud backup ready")`);
        assert.equal(await evaluate(reviewsCdp, `window.__cloudAppliedCount`), 0);
        assert.equal(await evaluate(reviewsCdp, `window.__cloudAutosaveStarted`), false);
        await evaluate(reviewsCdp, `window.dispatchEvent(new CustomEvent("panini:local-state-saved", { detail: { kind: "browser-gate-test" } }))`);
        await waitForExpression(reviewsCdp, `window.__cloudAutosaveStarted === true`);
        ...
        const queueAfterFailedLoad = await evaluate(reviewsCdp, `({
          ...
          loadButtonDisabled: document.querySelector("#restoreCloudIdButton")?.disabled,
        })`);
        assert.deepEqual(queueAfterFailedLoad, {
          codes: "TUR5",
          reviewVisible: true,
          restoreInputType: "password",
      ...
      await evaluate(cdp, `(() => {
        const completeAlbumCodes = ["USA1", "CUW9", "CUW13", "CIV10", "NED13", "SWE5", "EGY5", "EGY3", "CPV13", "IRQ6", "IRQ9", "IRQ19", "AUT13", "PAN5"];
        localStorage.setItem("panini.collectionTracker.v1", JSON.stringify({ collected: completeAlbumCodes }));
        const input = document.querySelector("#compareText");
        input.value = "Hello, I am looking for the last cards USA1, CUW9 and 13, CIV 10, NED13, SWE5, EGY5 et 3, CPV13, IRQ 6, 9 and 19, AUT 13, PAN 5. Lot of doubles to exchange";
        input.dispatchEvent(new Event("input", { bubbles: true }));
      })()`);
      await waitForExpression(cdp, `document.querySelector(".pasteCardStatusPreview [data-paste-card-summary]")?.textContent.startsWith("0 new for album")`);
      const conjunctionPreview = await evaluate(cdp, `({
        summary: document.querySelector(".pasteCardStatusPreview [data-paste-card-summary]").textContent,
        count: document.querySelector(".pasteCardStatusPreview [data-paste-card-count]").textContent,
        codes: [...document.querySelectorAll(".pasteCardStatusList li strong")].map((row) => row.textContent),
      })`);
      assert.equal(conjunctionPreview.summary, "0 new for album · 14 new trading cards · 0 duplicate trading cards");
      assert.equal(conjunctionPreview.count, "14 parsed");
      assert.equal(conjunctionPreview.codes.includes("AND13"), false);
      assert.equal(conjunctionPreview.codes.includes("AND19"), false);
      assert.equal(conjunctionPreview.codes.includes("ET3"), false);
      assert.deepEqual(conjunctionPreview.codes, [
        "AUT13", "CIV10", "CPV13", "CUW9", "CUW13", "EGY3", "EGY5",
        "IRQ6", "IRQ9", "IRQ19", "NED13", "PAN5", "SWE5", "USA1",
      ]);
      ...
      const cleanScannerPage = await createPage("about:blank");
      const cleanScannerCdp = await connectCdp(cleanScannerPage.webSocketDebuggerUrl);
      try {
        ...
        await send(cleanScannerCdp, "Page.addScriptToEvaluateOnNewDocument", {
          source: `sessionStorage.setItem("fifa-v2-controller-reload-build-6dcc142be40f", "1")`,
        });
        ...
        await evaluate(cleanScannerCdp, `window.dispatchEvent(new CustomEvent("panini:cloud-sync-applied", {
          detail: { profileId: ${JSON.stringify(profileId)} },
        }))`);
        const busyImmediately = await evaluate(cleanScannerCdp, `document.querySelector("#photoReviewPanel").getAttribute("aria-busy")`);
        assert.notEqual(busyImmediately, "true");
        await delay(500);
        const cleanScannerState = await evaluate(cleanScannerCdp, `({
          result: document.querySelector("#photoScannerResult").value,
          reviewHidden: document.querySelector("#photoReviewPanel").hidden,
          openReviewsHidden: document.querySelector("#photoOpenReviews").hidden,
        })`);
        assert.deepEqual(cleanScannerState, {
          result: "",
          reviewHidden: true,
          openReviewsHidden: true,
        });
      } finally {
        cleanScannerCdp.close();
        await withTimeout(
          fetch(`http://127.0.0.1:${DEBUG_PORT}/json/close/${cleanScannerPage.id}`),
          1000,
          "clean scanner tab did not close",
        ).catch(() => {});
      }
