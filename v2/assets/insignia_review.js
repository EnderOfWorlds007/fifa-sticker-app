...
async function init() {
  const response = await fetch("/fifa-sticker-app/v2/api/back-insignia-review", { cache: "no-store" });
...
async function decide(decision) {
  ...
  const response = await fetch("/fifa-sticker-app/v2/api/back-insignia-review/labels", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: item.id, decision, code: item.code, predicted_type: item.predicted_type, predicted_confidence: item.predicted_confidence }) });
