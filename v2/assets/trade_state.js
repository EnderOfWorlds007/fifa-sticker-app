...
export function extractCodeOccurrences(value) {
  ...
  for (const [name, team] of COUNTRY_NAME_CODES) {
    ...
    for (const match of upper.matchAll(groupedCountryPattern)) {
      ...
      for (const token of match[1].matchAll(groupedTokenPattern)) add(team, token[1], token[2], token[3] || token[4] || token[5]);
      countryNameSpans.push([match.index, match.index + match[0].length]);
    }
  }
  const zeroPattern = /(?<![A-Z0-9])00(?:\s*(?:\(\s*(?:(\d{1,2})\s*[X×]|[X×]\s*(\d{1,2}))\s*\)|[X×]\s*(\d{1,2})))?(?![A-Z0-9])/g;
  for (const match of upper.matchAll(zeroPattern)) {
    add("00", "", "", match[1] || match[2] || match[3]);
  }
  ...
  const looseGroupedPattern = /(?:^|\n)[\t ]*(?:[-*•][\t ]*)?([A-Z]{2,3})[\t ,-]*([1-9][0-9S\t ,/&+().X×-]*)(?=$|\n)/gm;
  for (const match of upper.matchAll(looseGroupedPattern)) {
    const start = match.index + match[0].indexOf(match[1]);
    if (
      countryNameSpans.some(([spanStart, spanEnd]) => spanStart <= start && start < spanEnd)
      || groupedSpans.some(([spanStart, spanEnd]) => spanStart <= start && start < spanEnd)
    ) continue;
    for (const token of match[2].matchAll(groupedTokenPattern)) add(match[1], token[1], token[2], token[3] || token[4] || token[5]);
    groupedSpans.push([start, match.index + match[0].length]);
  }
  for (const [name, team] of COUNTRY_NAME_CODES) {
    if (team === "00") continue;
    const namePattern = countryNamePattern(name);
    const inlineCountryPattern = new RegExp(`(?<![A-Z0-9])${namePattern}\\s*[-–—_./]?\\s*([1-9]\\d?)(S)?(?:\\s*(?:\\(\\s*(?:(\\d{1,2})\\s*[X×]|[X×]\\s*(\\d{1,2}))\\s*\\)|[X×]\\s*(\\d{1,2})))?(?![A-Z0-9])`, "g");
    for (const match of upper.matchAll(inlineCountryPattern)) {
      ...
      add(team, match[1], match[2], match[3] || match[4] || match[5]);
      countryNameSpans.push([match.index, match.index + match[0].length]);
    }
  }
  const inlinePattern = /(?<![A-Z0-9])([A-Z]{2,3})\s*[-–—_./]?\s*([1-9]\d?)(S)?(?:\s*(?:\(\s*(?:(\d{1,2})\s*[X×]|[X×]\s*(\d{1,2}))\s*\)|[X×]\s*(\d{1,2})))?(?![A-Z0-9])/g;
