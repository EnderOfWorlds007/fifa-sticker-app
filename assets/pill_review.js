...
async function init() {
  const response = await fetch("/fifa-sticker-app/api/pill-orientation-review", { cache: "no-store" });
  if (!response.ok) {
...
async function saveDecision(decision) {
  ...
  const item = decisions[currentIndex];
  const response = await fetch("/fifa-sticker-app/api/pill-orientation-review/labels", {
    method: "POST",
